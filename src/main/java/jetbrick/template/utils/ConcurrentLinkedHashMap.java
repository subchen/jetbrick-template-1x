...
import static jetbrick.template.utils.ConcurrentLinkedHashMap.DrainStatus.*;
import java.io.*;
import java.util.*;
...
public class ConcurrentLinkedHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
        ...
        void makeDead() {
            for (;;) {
                ...
                if (compareAndSet(current, dead)) {
                    weightedSize.set(weightedSize.get() - Math.abs(current.weight));
                    return;
                }
            }
        }
    ...
    static final class LinkedDeque<E extends Linked<E>> extends AbstractCollection<E> implements Queue<E>, Serializable {
